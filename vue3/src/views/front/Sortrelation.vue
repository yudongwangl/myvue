<template>
  <div class="box">
    <div class="lef">
      <div>
        ...
        <el-col :span="8" class="col">
          <el-dropdown trigger="click">
            <span class="el-dropdown-link" @click="addsort">添加<el-icon class="el-icon--right"><arrow-down /></el-icon></span>
            <template #dropdown>
              <el-dropdown-menu>
      ...
      <div v-for="item in mergedArray" :key="item.id" class="lef-bom" @click="handleClick(item.tid,item.img)">
...
import { upReply, getreso, getrelist,getretwo,addReply,adDsort } from '../../api/index'
...
const getupReply = async (id) => {
  const res = await upReply({ id: id });
  lefBoms.value = res.data.reply;
  replyData.uimg = lefBoms.value.find(item => item.head_img === item.head_img)?.head_img;   //加入用户头像
  const wwid = [...new Set(lefBoms.value.map(item => item.tid))]; //过滤消息
  console.log(wwid, 'wwid');  //查询用户分别联系了谁(教师)
  const resall = await Promise.all(wwid.map(id => getreso({ id: id })));
  mergedArray.value = resall.flatMap(item => item.data);
}

const getredata = async (tid) => {
  const res = await getrelist({ tid: tid });  //过滤分析
  topData.value = res.data.data;
  const pid = topData.value.find(item => item.tid === item.tid)?.tid;
  if (pid) {
    replyData.tid = pid;
    const res2 = await getretwo({ pid: pid });  //获取具体消息
...
const addusReply = async () => {
        ...
        const tid = replyData.tid;
        getredata(tid);
}
const addsortlist = async () => {
        const res = await adDsort();
        sortData.value = res.data.data;
        
}

const handleClick = (tid,img) => {
  getredata(tid);
  replyData.timg = img;   //加入教师头像
}
const addsort = () => {
  addsortlist();
}
